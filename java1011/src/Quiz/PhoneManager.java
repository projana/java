...
//		name = sc.next();
//		tel = sc.next();
//		phoneBook[1]= new Phone(name,tel);
//
//		System.out.println("이름과 전화번호 (번호는 연속적으로 입력)>> ");
//		name = sc.next();
//		tel = sc.next();
//		phoneBook[2]= new Phone(name,tel);

		System.out.println("저장되었습니다.");

//		배열 안에 있는 내용을 연속적으로 검색하여 출력하기 위해서 무한 루프를 사용함
		while (true) {
//
			...
			for (int i = 0; i < phoneBook.length; i++) {
//				getName() : Phone 클래스의 name 필드의 Getter메소드
...
//					변수에 사용자 입력내용과 배열에 저장된 내용이 같다는 의미로 논리값 입력
					flag = true;
					break; // for문 즉시 종료
				}
				else {
//					배열과 사용자 입력내용이 같지 않을 경우 false 입력
					flag = false;
				}
			}

//			flag 변수를 확인하여 값이 false이면  사용자가 입력한 내용과 배열의 모든 내용이 일치하지ㅏ않음
		if (flag == true){
