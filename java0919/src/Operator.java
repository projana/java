...
//        문자열: 문자의 배열을 뜻한다. 문자를 나열해 놓은 것.
//        - 자바의 문자열은 String 클래스 타입으로 레퍼런스 타입임.
//        - 자바의 기본 데이터 타입이 아니지만 워낙 많이 사용되는 데이터 타입이기 때문에 기본 데이터 타입처럼 사용 할 수 있음.
//        ㅇ-자바에서 문자열은 ""로 감싸여져 있는 모든 데이터를 문자열이라함.
...
//        "JDK10" +7이 나중에 연산되어 "JDK107" 이 됨
        String str3 ="JDK"+ 10 + 7;
        System.out.println(str3);  //JDK107

//        연산자의 연산 순서에 따라서 왼쪽에서 오른쪽으로 이동하면서 연산
//        10+7이 먼저 연산되어 17이 되과, 17+"JDK"가 나중에 연산되어 "17JDK"가 됨.
        String str4= 10+7 +"JDK";
        System.out.println(str4); //17JDK
...
//        처음에는 기본형태인 iVal1 = iVal +1;을 사용하는 것이 좋음
        iVal1 += 1;
        System.out.println(iVal1); //102

//        복합 대입 연산자인 += 은 문자열 연결 연산자로 많이 사용
//        JDBC 사용 시 자바 소스코드와 SQL 코드를 함께 사용 하게 됨
//        SQL코드를 문자열로 사용하기 때문에 이를 연결 하기 위해서 +=을 많이 사용
...
///      전치 증감 연산자로 동작하여 iVal6의 값을 증가하고 화면에 출력
        System.out.println("iVal6 :"+iVal6); //10
        System.out.println("iVal6 :"+ ++iVal6); //전치 11
//        후치 증감 연산자로 동작하여 iVal6의 값을 화면에 출력하고 나중에 값을 증가
        System.out.println("iVal6 :"+ iVal6); //11
        System.out.println("iVal6 :"+ iVal6++); // 후치 11
        System.out.println("iVal6 :"+ iVal6); //12
