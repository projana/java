...
//		매개변수로 받은 객체를 자식 클래스 탕브으로 각제 타입 변환

			Child2 child2  = (Child2) parent2; // 강제형변환
...
//		자식 클래스 객체 생성 후 부모 클래스 타입의 변수에 저장
		Parent2 parentA = new Child2(); // 원본이 자식 클래스
		method1(parentA);
		method2(parentA);

//		부모 클래스 타입으로 변수 선언
//		부모 클래스 객체 생성 후 부모 클래스 타입의 변수에 저장
		Parent2 parentB = new Parent2();// 원본이 부모 클래스
		method1(parentB);
//		method2(parentB);
	}
}
