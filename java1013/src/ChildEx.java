...
//		부모 클래스 타입의 인스턴스 멤버 사용
		parent.field1 = "parent date1";
		parent.method1();
		parent.method2();

		System.out.println("----------------------");

//		자식 클래스인 Child클래스 타입의 객체 생성
		Child child = new Child();
//		부모 클래스에서 상속받은 인스턴스 멤버 사용
//		자식 클래스 전용 인스턴스 멤버도 함께 사용
		child.field1 = "child data1"; // 상속받음
		child.field2 = "child data2"; // 전용멤버,전용멤버에 데이터 저장
		child.method1(); // 상속
		child.method2(); // 상속
		child.method3(); // 전용
...
//		부모 클래스 타입의 변수에 저장된 알맹이는 자식 클래스의 객체임
		parent = child;
		parent.field1 = "child date1"; // 부모의 인스턴스 멤버
//		parent.field2 = "chaild data2"; // 자식의 인스턴스 멤버 , 사용불가, 숨겨짐
...
//		자식 클래스 타입의 변수에 변환되었던  부모 클래스 타입의 ㄱ객체를 다시 자식 클래스 타입의 객체로 강제 타입 변환 후 저장
//		자식클래스 타입의 인스턴스 맴버를 모두 사용할 수 있음
		Child child2 = (Child) parent;
		System.out.println(child2.field2);
		child2.field1 = "child date1"; // 상속받음
		child2.field2 = "child date2"; // 전용
		child2.method1(); // 상속
		child2.method2(); // 상속
		child2.method3(); // 전용
