...
//        - int는 기본 데이터 타입이기 떄문에 데이터 타입의 기능이 부족함,int 데이터 타입에 기능을 추가하여 사용자에게 여러가지 편의성을 제공하기 위해서 만들어진 데이터 타입이 랩핑 타입임
//        배열 args에 저장된 데이터는 String타입이므로 산술연산이 불가능하기 때문에 정수로 변환
//        parseInt(문자열) : 문자열 데이터를 정수 데이터로 변환하는 메소드
//        - 문자열이지만 숫자인 문자열을 정수로 변환, 숫자가 큰 따옴표로 감싸진 형태의 문자열만 변환이 가능"100"
