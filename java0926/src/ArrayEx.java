...
//        - 동일한 데이터 타입의 데이터만 배열에 저장할 수있음
//        - 각각의 데이터를 구분하기 위해서 index 를 사용(index는 0부터 시작)
//        - 배열을 표시하는 기호로 []를 사용
//        - [] 안에 index번호를 사용하여 원하는 위치에 데이터를 저장하거나 꺼내어 사용할 수있음
//        - 배열은 선언 시 해당 배열의 크기를 설정하고, 지정한 크기를 벗어나서 사용할 수 없음
//        - 한번 설정된 배열의 크기는 변경할 수 없음
//        - 배열은 데이터 타입이 참조 타입이기 때문에 null로 초기화가 가능함  int[] arr1 = null;

//        사용법 : 선언
//        1. 데이터타입[] 배열명; // 배열을 메모리에 선언만 진행하고 나중에 필요한 곳에서 실제 배열을 생성함, 초기화 X.....입점예정ㅋㅋ
//        1. 배열명 = new 데이터타입[배열크기];  //  선언만 해놓은 배열을 실제로 사용하기 위해서 초기화
//       !2. 데이터타입[] 배열명 = new데이터타입[배열크기]; // 배열을 선언과 동시에 초기화까지 완료, 빈 배열
//        3. 데이터타입[] 배열명 = {데이터1, 데이터2, 데이터3, ...}; // 배열을 선언과 동시에 데이터를 추가함, 동시에 추가한 데이터의 갯수에               따라서 배열의 크기가 결정됨
//        4. 데이터타입[] 배열명 = new 데이터타입[] {데이터1, 데이터2, 데이터3,...}; // 배열을 선언과 동시에 데이터를 추가함, 동시에 추가한
//           데이터의 갯수에 따라서 배열의 크기가 결정됨
...
//        total = iu + you + inna + byoung + risa ;
//        avg = total /5;

        int total = score1 + score2 + score3 + score4 + score5;
        double avg = total / 5;

        System.out.println("학생 5명의 점수 총합은 : " + total + "점이고, 평균점수는 " + avg + "점 입니다.");
...
//        avg = total /5;
        ...
        // 배열을 메모리상에 선언만 했으므로 초기화가 되지않아서 배열의 내용으 ㄹ사용할 수없음
//        System.out.println(array1);
//        배열을 선언만 하고 초기화 하지 않았기 때문에 데이터의 저장이 불가능
//        array1[0] = 100;

//        메모리에 선언된 배열을 초기화함
        array1 = new int[5]; //초기화
//        초기화된 배열은 사용이 가능함
        System.out.println(array1);
//        배열을 초기화했으므로 데이터의 저장이 가능함
        array1[0] = 100;
//        배열이 초기화했으므로 데이터의 출력도 가능함
...
//        배열에 데이터를 저장하는 것에 문제 없음
        array3[0] = 100;
//        배열의 저장된 데이터를 가져오는 것에도 문제 없음
        System.out.println(array3[0]);
        System.out.println(array3[1]);

//        배열을 먼저 선언만 하고, 나중에 배열을 대입하여 저장할 경우
//        {데이터1, 데이터2, ...}  형식 나중에 대입하여 저장하는 방식은 오류가 발생
//        선언만 되어 있는 배열은 초기화가 되어있지 않기 때문에{데이터1, 데이터2,...}형태이 데이터를 단순 대입하는 것이 불가능함
//        int[] array31;
...
//        System.out.println(array4) ; // 배열이 초기화 되지않아서 사용 불가능
