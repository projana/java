...
//        Car  클래스 타입의 변수 myCar를 선언과 동시에 new 키워드를 실행하여 Car 클래스의 생성자를 호출하고 메모리에 등록된 객체의 주소를 받아와서 Car 클래스 타입의 변수 myCar에 주소를 저장
        Car myCar; // 변수
//       new  키워드를 사용하여 객체를 생성하지 않았기 때문에  Car클래스에서 제공하는 필드에 접근할 숭 없음
//        System.out.println(myCar.company);
        myCar = new Car(); // 객체생성

//       생성된 Car 클래스 타입의 객체 myCar 의 필드값을 출력
...
//        기존에 설정된 필드값을 수정
        ...
        String company = "KGM";
        String model; // 변수의 초기화 안함
        int maxSpeed = 250;
        int speed; // 변수의 초기화 안함
...
//        System.out.println(speed); //  초기화되지 않은 변수 사용 시 오류 발생
        ...
        System.out.println("\n----------------------\n");
//        같은 클래스를 사용하여 new  키워드로 객체를 생성하더라고도 서로 다른 메모리 주소를 사용함
//        각각의 객체끼리는 필드의 데이터를 공유하지 않음
//        객체 생성 시 서로 메모리가 달라지기 때문에 서로 완전히 다른 영역에 존재함
//        각각의 '객체명.필드명' 형태로 사용해야 정확한 데이터를 저장하고 사용할 수 있음
