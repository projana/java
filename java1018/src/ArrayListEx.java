...
//		add() 로 데이터 추가, ArrayList 객체의 끝에 추가함
		...
		list.add(2,"Database"); // add()의 첫번째 매개변수에 index를 사용 시 지정한  index에 데이터 추가
		list.add("iBatis");
	...
	int size = list.size();
		...
		String skill = list.get(2); //get()을 사용하여 지정한  index에 있는 데이터를 가져옴
		...
		list.remove(2); // remove() 를 사용하여 지정되 ㄴ index에 있는 데이터 삭제
		list.remove(2);
		list.remove("iBatis"); // remove()를 사용하여 지정된 데이터를 직접 삭제
